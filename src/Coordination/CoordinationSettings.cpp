...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Milliseconds, slow_member_backpressure_no_progress_timeout_ms, 30000, "How long the leader keeps waiting for a replica that makes no progress at all, while the slow member backpressure is switched on with the `bpon` four letter command, before leaving it behind. Progress means an accepted response: log entries taken, or a snapshot object saved, so a replica that is merely slow keeps resetting this and only one that is stuck stops being waited for. Give it room: a replica applying a large snapshot is quiet for a long time and is exactly the one worth waiting for. A replica the leader cannot reach at all is dropped straight away, without waiting for this. 0 means the leader never stops waiting for a replica it can reach.", 0) \
