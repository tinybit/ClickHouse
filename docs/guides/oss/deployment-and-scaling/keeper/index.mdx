...
| `slow_member_backpressure_no_progress_timeout_ms` | How long the leader keeps waiting for a replica that makes no progress at all, while the backpressure is on with [`bpon`](#four-letter-word-commands), before leaving it behind. Progress means an accepted response - log entries taken, or a snapshot object saved - so a replica that is merely slow keeps resetting this and only one that is stuck stops being waited for. A replica the leader cannot reach at all is dropped straight away, without waiting for this. `0` means the leader never stops waiting for a replica it can reach. | `30000` |
